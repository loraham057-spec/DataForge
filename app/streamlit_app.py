...
def run_script(script):
    """Exécute un script Python du dossier scraping."""
    ...
    st.success(f"✅ {script.name} terminé.")
    return True


# ============================================================
# 9. SCRAPING BOOKS
# Description : lance le scraper Books puis la récupération des
# détails complémentaires existante dans le projet.
# ============================================================

def scrape_books():
    if not run_script(BOOKS):
        return False

    if BOOKS_DETAILS.exists() and (CLEANED / "books_full.csv").exists():
        if not run_script(BOOKS_DETAILS):
            return False

    output = latest(BOOK_FILES)

    if output:
        st.success(f"📄 Résultat Books : {output.name}")

        try:
            count = sync_dataset(ROOT, "books", output)
            st.success(
                f"🗄️ Base SQL synchronisée : {count:,} lignes."
            )
        except Exception as exc:
            st.warning(
                f"⚠️ CSV produit mais synchronisation SQL impossible : {exc}"
            )

        return True

    st.warning("Aucun fichier Books produit.")
    return False


# ============================================================
# 10. SCRAPING GAARAAS
# Description : utilise le scraper de reprise s'il est disponible,
# afin de conserver le mécanisme déjà développé.
# ============================================================
...
def scrape_gaaraas():
    ...
    output = latest(GAARAAS_FILES)

    if output:
        st.success(f"📄 Résultat Gaaraas : {output.name}")

        try:
            count = sync_dataset(ROOT, "gaaraas", output)
            st.success(
                f"🗄️ Base SQL synchronisée : {count:,} lignes."
            )
        except Exception as exc:
    ...
    st.warning("Aucun fichier Gaaraas produit.")
    return False
...
if mode == "🚀 Scrape data":
    ...
    st.write(
        f"Source : **{source}** • "
        f"Pages configurées : **{pages}**"
    )

    st.warning(
        "Le paramètre de pages est transmis comme information "
        "d'interface ; les scripts Selenium existants conservent "
        "leur logique de pagination."
    )
